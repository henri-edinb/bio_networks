...
class LateralInhibitoryLayer:
    ...
    def gradient_step(self, add_feedback_y, bias_correction_y):
        gradient_y = add_feedback_y - (self.m @ self.y) + (np.random.normal(loc=0.0, scale=self.spont_act, size=(self.num_units_y))) - bias_correction_y - self.threshold
    ...
    def update_activations(self, current_input, train):
        ...
        self.current_input_y = (self.w @ self.current_input) + (self.p @ self.previous_y)
    ...
    def update_parameters(self, active_matrices):
        ...
        if "p" in active_matrices:
            ...
            #approximate stable state rule (objective derived rule to lead feedforward weigths to approximate dynamics)
            elif self.rule_p == 'perceptron': #delta pij = (y_i * previous_y_j) - (w_ij * previous_y_j^2)
                #raw_y = (self.w @ self.current_input) + (self.p @ self.previous_y)
                gradient_p = np.outer((self.y - (self.p @ self.previous_y)), self.previous_y)
            ...
            if self.dynamic_p or (self.rule_p == 'pehlevan'):
                self.p += (1/self.dynamic_learning_rate.reshape((self.num_units_y,1)))*gradient_w
            else:
